...
namespace Samples.StorageProviders
{
    ...
    public abstract class BaseJSONStorageProvider : IStorageProvider
    {
        ...
        {
            // Log = providerRuntime.gGetLogger(this.GetType().FullName);
            var typeResolver = providerRuntime.ServiceProvider.GetRequiredService<ITypeResolver>();
            this.serializerSettings = OrleansJsonSerializer.GetDefaultSerializerSettings(typeResolver, providerRuntime.GrainFactory);
